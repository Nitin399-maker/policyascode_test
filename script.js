...
async function saveRulesToSupabase(rules) {
  ...
  try {
    const rulesWithUserId = rules.map(rule => ({ ...rule, user_id: currentUser.id }));
    const { data, error } = await supabase.from('rules').upsert(rulesWithUserId, {onConflict: 'id'});
    if (error) {
      bootstrapAlert({ title: "Failed to save rules", body: error.message, color: "warning" });
    } else {
      bootstrapAlert({title:"Rules saved",body:`${rules.length} rule(s) saved to Supabase`, color: "success" });
    }
  } catch (error) {
...
async function connectToSupabase(url = null, key = null) {
  ...
  try {
    ...
    session ? handleUserAuth(session.user) : updateSupabaseUI();
    if (url || key) bootstrapAlert({title:"Supabase connected",body:"Connected to Supabase",color: "success" });
  } catch (error) {
    if (url || key) bootstrapAlert({ title: "Connection failed", body: "Failed to connect", color: "danger" });
  }
}

function handleUserAuth(user = null) {
  currentUser = user;
  updateSupabaseUI();
  if (user) bootstrapAlert({ title: "Signed in", body: `Welcome ${user.email}`, color: "success" });
}

async function handleGoogleAuth(signIn = true) {
  if (!supabase) {
    if(signIn)bootstrapAlert({title:"Not connected",body:"connect to Supabase first",color: "warning"});
    return;
  }
  try {
    if (signIn) {
      ...
      bootstrapAlert({ title: "Signed out", body: "You have been signed out", color: "info" });
    }
  } catch (error) {
    bootstrapAlert({ 
      title: signIn ? "Sign in failed" : "Sign out error", 
      body: signIn ?"Error signing in." : "Error signing out",color: signIn ? "danger" :"warning"});
  }
}

function updateSupabaseUI() {
  const supabaseStatus = $("#supabase-status");
  const supabaseConnect = $("#supabase-connect");
  const supabaseSignIn = $("#supabase-signin");
  const supabaseSignOut = $("#supabase-signout");
  const userEmail = $("#user-email");
  if (!supabase) {
    supabaseStatus.textContent = "Not connected";
    supabaseConnect.style.display = "inline-block";
    supabaseSignIn.style.display = "none";
    supabaseSignOut.style.display = "none";
    userEmail.textContent = "";
  } else if (!currentUser) {
    supabaseStatus.textContent = "Connected - Please sign in";
    supabaseConnect.style.display = "none";
    supabaseSignIn.style.display = "inline-block";
    supabaseSignOut.style.display = "none";
    userEmail.textContent = "";
  } else {
    supabaseStatus.textContent = "Connected & Authenticated";
    supabaseConnect.style.display = "none";
    supabaseSignIn.style.display = "none";
    supabaseSignOut.style.display = "inline-block";
...
async function processSource(source, name, isPdfBlob = false) {
  ...
  saveState();
  if (rules && rules.length > 0) { await saveRulesToSupabase(rules); }
}
...
async function consolidate() {
  ...
  if (edits && edits.length > 0) {
    ...
    const newMergedRules = [];     // Added: Track new merged rules for Supabase saving
    ...
    edits.forEach((edit) => {
      if (edit.edit === "merge") {
        ...
        newMergedRules.push(newRule); // Added: Track for Supabase
      }
    });
    ...
    if (newMergedRules.length > 0) { await saveRulesToSupabase(newMergedRules); }
  }
...
// Save edited rule
on(document, "click", "#saveRuleBtn", async () => { // Added: async for Supabase save
  ...
  if (form.checkValidity() && currentEditingRuleId) {
    ...
    redraw({});
    saveState();
    await saveRulesToSupabase([rule]);     // Added: Save updated rule to Supabase
